...
def rules():
    ...
    print("The positions of this 3 x 3 board is same as the right side of your ley board.\n")
    print(" 7 | 8 | 9 ")
    print("-----------")
    print(" 4 | 5 | 6 ")
    print("-----------")
...
def player_choice(board, name, choice):
    ...
    while position not in [1,2,3,4,5,6,7,8,9] or not space_check(board, position):
        position = int(input(f'\n{name} ({choice}), Choose your nest position: (1-9) \t'))
...
def replay():
    return input('\mDo you want to play again? Enter [Y]es or [N]o: ').lower().startswith('y')
...
while True:
    ...
    if mode == 1:
        ...
        p1_name = input("\nEnter NAME of PLAYER who will go againts the Computer:\t").capitalize()
        p2_name = "Computer"
        p1_choice, p2_choice = choice()
        print(f"\n{p1_name}:", p1_choice)
        print(f"{p2_name}:", p2_choice)
